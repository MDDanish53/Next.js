...
const PricingCard = ({
  ...
  const { has } = useAuth(); // to identify which plan the user is currently on

  const isCurrentPlan = id ? has?.({ plan: id }) : false; // to identify the current user plan is free(true) or paid(false)
  ...
  return (
    <div
      ref={ref}
      className={`relative backdrop-blur-lg border border-3xl p-8 transition-all rounded-3xl duration-700 cursor-pointer ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"} ${isHovered ? "transform scale-115 rotate-1 z-10" : ""} ${featured ? "bg-linear-to-b from-blue-500/20 to-purple-600/20 border-blue-400/50 scale-105" : "bg-white/5 border-white/10"}`}
...
const Pricing = () => {
  const plans = [
    ...
    },
    {
      ...
      featured: true,
      planId: "cplan_36KaH3gtEiFJmrbWRfVfkIEdxnI",
